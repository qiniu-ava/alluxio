...
public class KodoUnderFileSystem extends ObjectUnderFileSystem {
  ...
  private static final String FOLDER_SUFFIX = alluxio.Configuration.get(PropertyKey.UNDERFS_KODO_FOLDER_SUFFIX);
  ...
  private static Builder initializeKodoClientConfig(UnderFileSystemConfiguration conf) {
    ...
    String connectTimeout = conf.getValue(PropertyKey.UNDERFS_KODO_CONNECT_TIMEOUT);
    Matcher matcher = Pattern.compile("\\d+").matcher(connectTimeout);
    if (matcher.find()) {
      connectTimeout = matcher.group();
    }
    builder.connectTimeout(Long.valueOf(connectTimeout),TimeUnit.SECONDS);
    return builder;
  }
